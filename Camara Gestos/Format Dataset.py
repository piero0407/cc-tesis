...
def marcel():
    ...
    for f in teList:
        ...
        # Guardar imagen en arreglo de imagenes
        dataset[labelIndex].append(image)
        # Guardar etiqueta en arreglo de etiquetas
        labels[labelIndex].append(labelToVector(f.stem[0]))
    ...
    dsguardar = [(np.array(x_trn), np.array(y_trn)),
...
def dataset():
    ...
    for f in teList:
        fileName = str(f)
        labelIndex = labelToIndex(f.stem[1])
        # Imagen a escala de grises
        image = cv2.imread(fileName, flags=cv2.IMREAD_GRAYSCALE)
        image = cv2.resize(image, dsize=(numPix, numPix)).reshape(
            (1, numPix * numPix))  # Imagen en matris (100x100) a vector (1x10000)
        ...
        dataset[labelIndex].append(image)
        # Guardar etiqueta en arreglo de etiquetas
        labels[labelIndex].append(labelToVector(f.stem[1]))
    ...
    for i in range(len(dataset)):
        ...
        y_trni = np.array(labels[i][:m_trni]).reshape(
            m_trni, len(datasetLabels))

        m_vali = int(len(dataset[i]) * .2)
        x_vali = np.array(dataset[i][m_trni:m_trni + m_vali]
                          ).reshape(m_vali, numPix * numPix)
        y_vali = np.array(labels[i][m_trni:m_trni + m_vali]
                          ).reshape(m_vali, len(datasetLabels))
